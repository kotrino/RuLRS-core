rem --  Изменение параметров RuLRS

rem --  Лицензия https://www.gnu.org/licenses/gpl-3.0.en.html

rem --  Базовый скрипт для радио с ErskTx R223A6 или новее

rem -- Автор оригинальной версии: spinorkit
rem -- Адаптация для RuLRS: kotrino

rem -- Ссылки на документацию:
rem -- Описание диалекта ErskyTx Basic: https://openrcforums.com/forum/viewtopic.php?t=10325
rem -- N.B. ErskyTx Basic использует индексацию массивов с 0, в отличие от lua с индексацией с 1

rem -- Примечание: ErskyTx использует 400000 бод для связи с модулями crossfire и RuLRS.
rem -- Если у вас модуль 2018 R9M (с логотипом ACCST на задней панели), вам нужна 
rem -- модификация резистора для надежной связи:
...
main:
   ...
   ygap = 10
   yPos = 0
   drawtext(0, yPos, "RuLRS", INVERS)
   if wifiupdatemode = 2
      drawtext(30, yPos, "http://10.0.0.1")
   elseif bindMode = 1
      drawtext(30, yPos, "Binding not needed")
   else
   rem UartBadPkts = 250
