...
def process_http_result(output_json_file: str) -> int:
    ...
    # Returns: RulrsUploadResult enum
    ...
    if result == 'ok':
        ...
        retval = RulrsUploadResult.Success
        ...
        retval = RulrsUploadResult.ErrorMismatch
    else:
        # Not enough space.
        msg = f'UPLOAD ERROR\n\033[31m{msg}\033[0m' # red
        retval = RulrsUploadResult.ErrorGeneral
...
def do_upload(rulrs_bin_target, pio_target, upload_addr, env):
    bootloader_target = None
    app_start = 0 # eka bootloader offset

    bin_upload_output = os.path.splitext(rulrs_bin_target)[0] + '-output.json'
    if os.path.exists(bin_upload_output):
        os.remove(bin_upload_output)

    cmd = ["curl", "--max-time", "60",
           "--retry", "2", "--retry-delay", "1",
           "--header", "X-FileSize: " + str(os.path.getsize(rulrs_bin_target)),
    ...
    if pio_target == 'uploadforce':
        cmd += ["-F", "force=1"]
    if do_bin_upload:
        cmd += "-F", "data=@%s" % (rulrs_bin_target),

    upload_port = env.get('UPLOAD_PORT', None)
    if upload_port is not None:
        upload_addr = [upload_port]

    returncode = RulrsUploadResult.ErrorGeneral
    for addr in upload_addr:
        ...
        if returncode == RulrsUploadResult.Success:
            return returncode
...
def on_upload(source, target, env):
    ...
    upload_addr = ['rulrs_tx', 'rulrs_tx.local']
    rulrs_bin_target = os.path.join(bin_path, 'firmware.rulrs')
    if not os.path.exists(rulrs_bin_target):
        rulrs_bin_target = os.path.join(bin_path, 'firmware.bin.gz')
        if not os.path.exists(rulrs_bin_target):
            rulrs_bin_target = os.path.join(bin_path, 'firmware.bin')
            if not os.path.exists(rulrs_bin_target):
                raise Exception("No valid binary found!")

    pio_target = target[0].name
    return do_upload(rulrs_bin_target, pio_target, upload_addr, env)
