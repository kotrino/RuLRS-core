...
---- #########################################################################
---- #                                                                       #
---- # Copyright (C) OpenTX, адаптировано для RuLRS                          #
-----#                                                                       #
---- # Лицензия GPLv2: http://www.gnu.org/licenses/gpl-2.0.html             #
---- #                                                                       #
---- #########################################################################
local deviceId = 0xEE
...
local ALLOWED_SERIAL_NUMBERS = {
    [0x454C5253] = true,  -- 'ELRS'
    [0x524C5253] = true,  -- 'RLRS'
}

-- Выбрать следующее или предыдущее редактируемое поле
...
local function fieldGetStrOrOpts(data, offset, last, isOpts)
  -- Для isOpts: Разделить таблицу байтовых значений (строку) с разделителем ; на таблицу
  -- Иначе просто читать строку до первого нулевого байта
  ...
  repeat
    ...
    if not last then
      ...
      elseif b ~= 0 then
        -- Для систем с определенными константами стрелок использовать их
        -- вместо символов \xc0 \xc1 (которые используются в английской версии OpenTX)
        -- Если символ есть в таблице замен - использовать его, иначе конвертировать через string.char
        opt = opt .. (({
...
local function fieldUnsignedLoad(field, data, offset, size, unitoffset)
  ...
  -- Сохранять размер поля только если он не равен 1 
  -- (это охватывает большинство числовых полей и полей выбора (fields / selection))
  if size ~= 1 then
    field.size = size
  end
end
...
local function fieldSignedLoad(field, data, offset, size, unitoffset)
  ...
  -- Отрицательный размер поля указывает на то, что это знаковое целое число
  field.size = -size
end
...
local function fieldFloatLoad(field, data, offset)
  ...
  -- Преобразовать значение точности в делитель для отображения
...
local function fieldTextSelLoad(field, data, offset)
  ...
  -- Если в списке значений только один вариант - сделать поле неактивным
  if not cached then
    field.grey = vcnt <= 1
  end
  ...
  if deviceId == id then
    ...
    if newFieldCount ~= fields_count or newFieldCount == 0 then
      ...
      if newFieldCount == 0 then
        -- У этого устройства нет полей, поэтому код загрузки никогда не запускается
        createDeviceFields()
      end
    end
  end
...
local function parseParameterInfoMessage(data)
  ...
  if chunksRemain > 0 then
    ...
    -- Последнее поле загружено, добавляем список устройств в конец
    if #loadQ == 0 then
      createDeviceFields()
    end

    -- Возвращаемое значение определяет, нужно ли обновлять экран
    -- Если deviceId является TX модулем, то Bad/Good управляет обновлением; для других
    -- устройств обновляем каждый новый элемент и всегда обновляем когда очередь пуста
...
local function parseRulrsInfoMessage(data)
  ...
  -- Если флаги меняются, сбрасываем таймаут предупреждения для немедленного отображения/скрытия сообщения
  if newFlags ~= rulrsFlags then
    rulrsFlags = newFlags
    titleShowWarnTimeout = 0
  end
  rulrsFlagsInfo = fieldGetStrOrOpts(data, 7)

  local state = (bit32.btest(rulrsFlags, 1) and "C") or "-"
...
local function refreshNext()
  ...
  repeat
    ...
    elseif command == 0x2D then
      parseRulrsV1Message(data)
    elseif command == 0x2E then
      parseRulrsInfoMessage(data)
      forceRedraw = true
    end
  until command == nil
  ...
  elseif time > linkstatTimeout then
    if not deviceIsRULRS_TX and #loadQ == 0 then
      goodBadPkt = ""
    else
  ...
  if time > titleShowWarnTimeout then
    -- if rulrsFlags bit set is bit higher than bit 0 and bit 1, it is warning flags
    titleShowWarn = (rulrsFlags > 3 and not titleShowWarn) or nil
...
local function lcd_title_color()
  ...
  if titleShowWarn then
    lcd.drawText(COL1 + 1, 4, rulrsFlagsInfo, CUSTOM_COLOR)
  else
...
local function lcd_title_bw()
  ...
  if #loadQ > 0 and fields_count > 0 then
    ...
    if titleShowWarn then
      lcd.drawText(COL1, 1, rulrsFlagsInfo, INVERS)
    else
      local title = fields_count > 0 and deviceName or "Loading..."
...
local function lcd_warn()
  ...
  lcd.drawText(COL1, textSize*3, rulrsFlagsInfo)
  lcd.drawText(LCD_W/2, textSize*5, "[OK]", BLINK + INVERS + CENTER)
end

local function reloadRelatedFields(field)
  -- Перезагрузить родительскую папку для обновления описания
  if field.parent then
    loadQ[#loadQ+1] = field.parent
    fields[field.parent].name = nil
  end

  -- Перезагрузить все редактируемые поля текущего уровня и родительское поле
  for fieldId = fields_count, 1, -1 do
    -- Пропустить текущее поле, оно будет добавлено последним
    local fldTest = fields[fieldId]
    local fldType = fldTest.type or 99 -- тип может быть nil если все еще загружается
    if fieldId ~= field.id
  ...
  -- Reload this field
  loadQ[#loadQ+1] = field.id
  -- Пауза, чтобы EEPROM успел записать данные
  fieldTimeout = getTime() + 20
end

local function handleDevicePageEvent(event)
  if #fields == 0 then --если полей fields еще нет
    return
  else
    if fields[#fields].name == nil then --если кнопка "Назад" еще не создана, значит загрузка полей fields не завершена
      return
    end
  end
...
local function setLCDvar()
  ...
  -- Определение версии popupConfirmation (передает 2 или 3 аргумента)
  -- if pcall(popupConfirmation, "", "", EVT_VIRTUAL_EXIT) then
  -- Версия 1 считается FreedomTX
...
local function setMock()
  ...
  local mock = loadScript("mockup/rulrsmock.lua")
  if mock == nil then return end
  fields, goodBadPkt, deviceName = mock()
  fields_count = #fields - 1
  loadQ = { fields_count }
  deviceIsRULRS_TX = true
end

local function checkCrsfModule()
  -- Поиск модуля CRSF (тип 5) среди доступных модулей
  for modIdx = 0, 1 do
    local mod = model.getModule(modIdx)
    if mod and (mod.Type == nil or mod.Type == 5) then
      -- Модуль CRSF обнаружен
      checkCrsfModule = nil
      return 0
    end
  end

  -- Модуль CRSF не найден, сохранить сообщение об ошибке для run()
  lcd.clear()
  local y = 0
  lcd.drawText(2, y, "  No RuLRS", MIDSIZE)
  y = y + (textSize * 2) - 2
  local msgs = {
